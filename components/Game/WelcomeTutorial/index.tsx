...
const WelcomeTutorial: FC<WelcomeTutorialProps> = ({
  show,
  mounted,
  clickOverlay,
}) => {
    ...
    <>
      ...
      <WTContainer show={show} mounted={mounted}>
        ...
        <WTWindow show={show} theme={themeColor}>
          <HowToPlayText theme={themeColor}>HOW TO PLAY</HowToPlayText>
          <RuleText>
            1. Two players alternate turns selecting a grid to open
          </RuleText>
          <RuleText>2. Bombs are randomly assigned to the grids</RuleText>
          <RuleText>
            3. A turn end once a grid is selected OR timer runs out
          </RuleText>
          <RuleText>4. Game ends once all bombs are revealed</RuleText>
          <Div3>
            <Bomb src="/game/bomb.svg" alt="bomb" />
            <BottomText left={'15px'} color={'white'}>
              = 1 pts
            </BottomText>
            <BottomText left={'20px'} color={themeColor.highlight}>
              Player with the most pts wins
            </BottomText>
          </Div3>
        </WTWindow>
        <ReturnText>Click anywhere to continue</ReturnText>
      </WTContainer>
    </>
  )
}
